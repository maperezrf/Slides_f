...
class CLASSIFIER_F4():
    ...
    def filters(self):
        ...
        self.f4_db_reg = self.f4.loc[(self.f4[var_f4["tipo_redinv"]] == "dado de baja") & (self.f4[var_f4['estado']] =='registrado') & (self.f4[var_f4["fecha_res"]] >= "2022-01-01")].reset_index(drop=True)
    ...
    def set_posible_causa(self):
        ...
        self.f4_db_res.loc[self.f4_db_res['Posible Causa'].isna() & (self.f4_db_res.local_agg =='TIENDA') & self.f4_db_res[var_f4["destino"]].str.contains(r'[1][1]\d{7,}'), 'Posible Causa'] = 'Cierre de F11 - Tienda'
        ...
        self.f4_db_res.loc[self.f4_db_res['Posible Causa'].isna() & (self.f4_db_res.local_agg =='DVD ADMINISTRATIVO') & (self.f4_db_res[var_f4["destino"]].str.contains(r'[1][1]\d{7,}')), 'Posible Causa'] = 'Cierre de F11 - DVD'
        ...
        self.f4_db_res.loc[self.f4_db_res['Posible Causa'].isna() & (self.f4_db_res.local_agg =='CD') & (self.f4_db_res[var_f4["destino"]].str.contains(r'[1][1]\d{7,} sin recupero|[1][2]\d{7,} sin recupero')), 'Posible Causa'] = 'Cierre de F11 - CD'
    ...
    def save_files(self):
        ...
        if self.reg_sin_marca > 0:
            ...
            pd.DataFrame(upc,columns=["UPC"]).to_excel(f"output/{self.dt_string}_UPCs_nuevos.xlsx", index=False)

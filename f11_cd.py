...
class F11():
    ...
    def f11_filters(self):
        ...
        f11_cd = fltr_grupo(f11_emp_abiertos)
        return f11_cd
    ...
    def f11_resfil(self):
        # Gráfica para costo
        gb_f11_gm = self.f11_rf.groupby([dtagco['servicio'], dtagco['mes']], sort=False)[dtagco['costo']].sum().reset_index() # TODO repetido 
        gb_f11_gm = set_columns_sum(gb_f11_gm, dtagco['mes'],dtagco['costo'])
        gb_f11_gm = set_columns_sum(gb_f11_gm, dtagco['servicio'],dtagco['costo'])
        orden_grupo = gb_f11_gm.groupby([dtagco['servicio']], sort=False)[dtagco['costo']].sum().sort_values(ascending=False).reset_index()[dtagco['servicio']].to_list()
        orden_mes = gb_f11_gm[dtagco['mes']].unique().tolist()
        total_abierto = gb_f11_gm[dtagco['costo']].sum()
        gb_f11_3m_grupo = self.f11_m90.groupby(dtagco['servicio'])[dtagco['costo']].sum().reset_index().set_index(dtagco['servicio']) # Calcula los totales de costo por grupo 
        self.fig_f11_costo(gb_f11_gm, gb_f11_3m_grupo, orden_grupo, orden_mes, total_abierto)

        # Gráfica por cantidad 
        gb_f11_gm_cant = self.f11_rf.groupby([dtagco['servicio'], dtagco['mes']], sort=False)[dtagco['f11_id']].nunique().reset_index() # TODO repetido
        gb_f11_gm_cant = set_columns_nunique(gb_f11_gm_cant, dtagco['mes'],dtagco['f11_id'])
        gb_f11_gm_cant = set_columns_nunique(gb_f11_gm_cant, dtagco['servicio'],dtagco['f11_id'])
        orden_grupo_cant = gb_f11_gm_cant.groupby([dtagco['servicio']], sort=False)[dtagco['f11_id']].sum().sort_values(ascending=False).reset_index()[dtagco['servicio']].to_list()
        orden_mes_cant = gb_f11_gm_cant[dtagco['mes']].unique().tolist()
        total_abierto_cant = gb_f11_gm_cant[dtagco['f11_id']].sum()
        gb_f11_3m_cant_grupo = self.f11_m90.groupby(dtagco['servicio'])[dtagco['f11_id']].nunique().reset_index().set_index(dtagco['servicio']) # Calcula los totales de costo por grupo 
    ...
    def fig_f11_costo(self, df, gb_annotations, orden_grupo, orden_mes, ta):
        f11_empresa_sede = px.bar(df, x=dtagco['mes'], y=dtagco['costo'], color=dtagco['servicio'], text=dtagco['costo'], text_auto='.2s', category_orders={dtagco['servicio']:orden_grupo, dtagco['mes']:orden_mes})
        f11_empresa_sede.update_layout(barmode='stack',title_text=f"F11s empresa abiertos CD - Total abierto {ta/1e6:,.0f}M") #,uniformtext=dict(mode="hide", minsize=10),legend=dict(yanchor="top", y=0.95, xanchor="left", x=0.1))
        ...
        mes_ref = orden_mes[0] 
        f11_empresa_sede.add_annotation(x=mes_ref, y=5*1e8, text= f"Total > 90 días = {gb_annotations.sum()[0]/1e6:,.0f}M", showarrow=False, font = dict (color = "red",size = 17), xanchor='left') # TODO Estas líneas pueden agrupar, en un solo add_annotation, utilizando <br>, y se alinea mejor utilizando fig.update_annotations(align="left") 
        #f11_empresa_sede.add_annotation(x=mes_ref, y=7*1e8, text= f"CD = {gb_annotations.loc['CD'][0]/1e6:,.0f}M",showarrow=False,font = dict (color = "red",size = 12), xanchor='left')
    ...
    def fig_f11_cantidad(self, df, gb_annotations, orden_grupo, orden_mes, ta):
        f11_es_cantidad = px.bar(df, x=dtagco['mes'], y=dtagco['f11_id'], color=dtagco['servicio'], text=dtagco['f11_id'], text_auto='.0f', category_orders={dtagco['servicio']:orden_grupo, dtagco['mes']:orden_mes})
        f11_es_cantidad.update_layout(barmode='stack',title_text=f"F11s empresa abiertos CD - Total abierto {ta:,.0f} folios de F11" ) #,uniformtext=dict(mode="hide", minsize=10),legend=dict(yanchor="top", y=0.95, xanchor="left", x=0.1))
        f11_es_cantidad.update_layout(legend=dict(orientation="h", yanchor="top", y=-0.2, xanchor="right", x=0.5))
        f11_es_cantidad.add_shape(type="rect",xref="paper", yref="paper",x0=0, y0=0,x1=0.81, y1=0.98,line=dict(color="red", width=2,))
        mes_ref = orden_mes[0]
        f11_es_cantidad.add_annotation(x=mes_ref, y=800, text= f"Total > 90 días = {gb_annotations.sum()[0]:,.0f} folios", showarrow=False, font = dict (color = "red",size = 17), xanchor='left') # TODO igual que en la lina 88
        #f11_es_cantidad.add_annotation(x=mes_ref, y=700,text= f"CD = {gb_annotations.loc['CD'][0]:,.0f} folios",showarrow=False,font = dict (color = "red",size = 12), xanchor='left')
...
def fltr_grupo(df):
    return df.loc[df[dtagco['grupo']] == 'CD'].reset_index(drop=True)
